...
# Mixed columns lookup tables, encryption
...
def rotate_word_left(word, positions):
...
# Encrypt each block separately, then send out to Standard output in aggregate, in base_64
def cryptopals_encrypt(key_in_bytes, plain_in_bytes, initialization_vector):
    keys = cryptopals_key_expansion_int(key_in_bytes)

    start = 0
    finish = 16

    return_in_aggregate = b''

    block = plain_in_bytes[start: finish]       
    block_int = int.from_bytes(block, 'big') ^ int.from_bytes(initialization_vector, "big")
    ...
    # while (block):
    while (finish <= len(plain_in_bytes)):
        ...
        return_in_aggregate += give_back
        
        start += 16
        finish += 16
        block = plain_in_bytes[start: finish] 
        block_int = int.from_bytes(block, 'big') ^ int.from_bytes(give_back, "big")
        block_word_1 = (block_int & 0xffffffff000000000000000000000000) >> 96  
        block_word_2 = (block_int & 0x00000000ffffffff0000000000000000) >> 64
        block_word_3 = (block_int & 0x0000000000000000ffffffff00000000) >> 32
        block_word_4 = block_int & 0x000000000000000000000000ffffffff
    
    # sys.stdout.buffer.write(base64.b64encode(return_in_aggregate))
    # return base64.b64encode(return_in_aggregate)
    return return_in_aggregate
...
def shift_rows_block(word1, word2, word3, word4):
    ...
    transpose2_shifted = rotate_word_left(transpose2, 1)
    transpose3_shifted = rotate_word_left(transpose3, 2)
    transpose4_shifted = rotate_word_left(transpose4, 3)
...
# Rotates word x positions right
def rotate_word_right(word, positions):
    give_back = ((word >> (8 * positions)) | (word << (32 - (8 * positions)))) & 0xffffffff
    return give_back

# Substitutes with the inverse s box
def inverse_substitute_word_int(word):
    byte1 = InvSbox[(word >> 24) & 0xffffffff] << 24
    byte2 = InvSbox[(word >> 16) & 0x00ff] << 16 
    byte3 = InvSbox[(word >> 8) & 0x0000ff] << 8
    byte4 = InvSbox[word & 0x000000ff]
    return (byte1 | byte2 | byte3 | byte4)

# Takes 4 words and substitutes them with inverse Sbox
def inverse_substitute_blocks(word1, word2, word3, word4):
    return [inverse_substitute_word_int(word1), inverse_substitute_word_int(word2), inverse_substitute_word_int(word3), inverse_substitute_word_int(word4)]

# inverse of shift rows
def inverse_shift_rows_block(word1, word2, word3, word4):
    ...
    transpose2_shifted = rotate_word_right(transpose2, 1)
    transpose3_shifted = rotate_word_right(transpose3, 2)
    transpose4_shifted = rotate_word_right(transpose4, 3)
...
def inverse_mix_columns_block(word1, word2, word3, word4):
    word_1_1 = (mul_14[(word1 & 0xff000000) >> 24] ^ mul_11[((word1 & 0x00ff0000) >> 16)] ^ mul_13[((word1 & 0x0000ff00) >> 8)] ^ mul_09[(word1 & 0x000000ff)]) << 24
    word_1_2 = (mul_09[((word1 & 0xff000000) >> 24)] ^ mul_14[((word1 & 0x00ff0000) >> 16)] ^ mul_11[((word1 & 0x0000ff00) >> 8)] ^ mul_13[(word1 & 0x000000ff)]) << 16
    word_1_3 = (mul_13[((word1 & 0xff000000) >> 24)] ^ mul_09[((word1 & 0x00ff0000) >> 16)] ^ mul_14[((word1 & 0x0000ff00) >> 8)] ^ mul_11[(word1& 0x000000ff)]) << 8
    word_1_4 = mul_11[((word1 & 0xff000000) >> 24)] ^ mul_13[((word1& 0x00ff0000) >> 16)] ^ mul_09[((word1& 0x0000ff00) >> 8)] ^ mul_14[(word1& 0x000000ff)]
    finished1 = word_1_1 | word_1_2 | word_1_3 | word_1_4

    word_2_1 = (mul_14[(word2 & 0xff000000) >> 24] ^ mul_11[((word2 & 0x00ff0000) >> 16)] ^ mul_13[((word2 & 0x0000ff00) >> 8)] ^ mul_09[(word2 & 0x000000ff)]) << 24
    word_2_2 = (mul_09[((word2 & 0xff000000) >> 24)] ^ mul_14[((word2 & 0x00ff0000) >> 16)] ^ mul_11[((word2 & 0x0000ff00) >> 8)] ^ mul_13[(word2 & 0x000000ff)]) << 16
    word_2_3 = (mul_13[((word2 & 0xff000000) >> 24)] ^ mul_09[((word2 & 0x00ff0000) >> 16)] ^ mul_14[((word2 & 0x0000ff00) >> 8)] ^ mul_11[(word2 & 0x000000ff)]) << 8
    word_2_4 = mul_11[((word2 & 0xff000000) >> 24)] ^ mul_13[((word2 & 0x00ff0000) >> 16)] ^ mul_09[((word2 & 0x0000ff00) >> 8)] ^ mul_14[(word2 & 0x000000ff)]
    finished2 = word_2_1 | word_2_2 | word_2_3 | word_2_4

    word_3_1 = (mul_14[(word3 & 0xff000000) >> 24] ^ mul_11[((word3 & 0x00ff0000) >> 16)] ^ mul_13[((word3 & 0x0000ff00) >> 8)] ^ mul_09[(word3 & 0x000000ff)]) << 24
    word_3_2 = (mul_09[((word3 & 0xff000000) >> 24)] ^ mul_14[((word3 & 0x00ff0000) >> 16)] ^ mul_11[((word3 & 0x0000ff00) >> 8)] ^ mul_13[(word3 & 0x000000ff)]) << 16
    word_3_3 = (mul_13[((word3 & 0xff000000) >> 24)] ^ mul_09[((word3 & 0x00ff0000) >> 16)] ^ mul_14[((word3 & 0x0000ff00) >> 8)] ^ mul_11[(word3 & 0x000000ff)]) << 8
    word_3_4 = mul_11[((word3 & 0xff000000) >> 24)] ^ mul_13[((word3 & 0x00ff0000) >> 16)] ^ mul_09[((word3 & 0x0000ff00) >> 8)] ^ mul_14[(word3 & 0x000000ff)]
    finished3 = word_3_1 | word_3_2 | word_3_3 | word_3_4

    word_4_1 = (mul_14[(word4 & 0xff000000) >> 24] ^ mul_11[((word4 & 0x00ff0000) >> 16)] ^ mul_13[((word4 & 0x0000ff00) >> 8)] ^ mul_09[(word4 & 0x000000ff)]) << 24
    word_4_2 = (mul_09[((word4 & 0xff000000) >> 24)] ^ mul_14[((word4 & 0x00ff0000) >> 16)] ^ mul_11[((word4 & 0x0000ff00) >> 8)] ^ mul_13[(word4 & 0x000000ff)]) << 16
    word_4_3 = (mul_13[((word4 & 0xff000000) >> 24)] ^ mul_09[((word4 & 0x00ff0000) >> 16)] ^ mul_14[((word4 & 0x0000ff00) >> 8)] ^ mul_11[(word4 & 0x000000ff)]) << 8
    word_4_4 = mul_11[((word4 & 0xff000000) >> 24)] ^ mul_13[((word4 & 0x00ff0000) >> 16)] ^ mul_09[((word4 & 0x0000ff00) >> 8)] ^ mul_14[(word4 & 0x000000ff)]
    finished4 = word_4_1 | word_4_2 | word_4_3 | word_4_4

    return [finished1, finished2, finished3, finished4]

# Cryptopals expansion
def cryptopals_key_expansion_int(key_in_bytes):
    block_int = int.from_bytes(key_in_bytes, 'big')
    ...
    round_increment = 0
    while (increment < 43):
        xored = words_int[increment - 4] ^ round_xor_int(substitute_word_int(rotate_word_left(words_int[increment - 1], 1)), round_increment)
...
# Takes key in byte string format and ciphertext in base64 read from file
def cryptopals_decrypt(key_in_bytes, cipher_in_base64, initialization_vector):
    keys = cryptopals_key_expansion_int(key_in_bytes)
    data_in_bytes = base64.b64decode(cipher_in_base64)
    initialization_vector_int = int.from_bytes(initialization_vector, "big")
    first = True
    ...
    block = data_in_bytes[start: finish]       
    ...
    while (finish <= len(data_in_bytes)):
        lst = add_round_key_block(block_word_1, block_word_2, block_word_3, block_word_4, keys[40], keys[41], keys[42], keys[43])
        for d in range(4, 40, 4):
            lst = inverse_shift_rows_block(lst[0], lst[1], lst[2], lst[3])
            lst = inverse_substitute_blocks(lst[0], lst[1], lst[2], lst[3])     
            lst = add_round_key_block(lst[0], lst[1], lst[2], lst[3], keys[43 - d - 3], keys[43 - d - 2], keys[43 - d - 1], keys[43 - d])    
            lst = inverse_mix_columns_block(lst[0], lst[1], lst[2], lst[3])

        lst = inverse_shift_rows_block(lst[0], lst[1], lst[2], lst[3])
        lst = inverse_substitute_blocks(lst[0], lst[1], lst[2], lst[3])           
        lst = add_round_key_block(lst[0], lst[1], lst[2], lst[3], keys[0], keys[1], keys[2], keys[3])

        # all other rounds, xor with old plaintext
        if (first == False):
            old = data_in_bytes[start - 16: finish - 16]       
            old_int = int.from_bytes(old, 'big')
            lst = [((old_int & 0xffffffff000000000000000000000000) >> 96) ^ lst[0], ((old_int & 0x00000000ffffffff0000000000000000) >> 64) ^ lst[1], ((old_int & 0x0000000000000000ffffffff00000000) >> 32) ^ lst[2], (old_int & 0x000000000000000000000000ffffffff) ^ lst[3]]
            give_back = bytes(list(itertools.chain.from_iterable(divide(lst))))
            # sys.stdout.buffer.write(give_back)
            return_in_aggregate += give_back
        
        # First round, xor with IV
        if (first == True):
            lst = [((initialization_vector_int & 0xffffffff000000000000000000000000) >> 96) ^ lst[0], ((initialization_vector_int & 0x00000000ffffffff0000000000000000) >> 64) ^ lst[1], ((initialization_vector_int & 0x0000000000000000ffffffff00000000) >> 32) ^ lst[2], (initialization_vector_int & 0x000000000000000000000000ffffffff) ^ lst[3]]
            give_back = bytes(list(itertools.chain.from_iterable(divide(lst))))
            first = False
            # sys.stdout.buffer.write(give_back)
            return_in_aggregate += give_back
...
def main():
    # Password comes in byte string format
